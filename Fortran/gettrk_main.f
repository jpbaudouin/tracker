...
c
c   16-05-16 Baudouin   - Change in the subroutine tracker to get the 
c                         genesis vitals file (fort.67) give the data for
c                         the last leadtime instead of the first
c
c   16-05-24 Baudouin   - Correction of an error concerning when 
c                         first_ges_center is called. It must be called 
c                         just after the storms tracked at the previous 
c                         time step. I updated the value of prevstormct
c                         and change the variable tested (sl_counter
c                         instead of ist)
c
c   16-06-13 Baudouin   Add the possibility to read netcdf files
c                       - Add the parameter : filetype in the namelist 
c                         (see gettrk_module.f)
c                       - Add the subroutines open_netcdf_files
c                                             getgridinfo_netcdf
c                                             getdata_netcdf
c
c   16-07-28 Baudouin  Genesis vitals file output at the penultimate 
c                      time step
c
c   16-07-29 Baudouin  Correct a bug when computing xlondegmove_last,
c                      that leaded to a bad extrapolation when track
c                      just pass through the greenwich meridian
c
c   17-10-10 Baudouin  Zeta 850 not considered for fixcenter subrtoutine L.1211
c                      max_mslp_850 change to 1000 instead of 400 L.1927
c                      (Other changes prior to this one not mentionned)
c
c                      To do : check ibeg / iend with different definition of grids
c
c
c Input files:
...
c
      ...
      integer ncid ! same role as lugb but value given by nf_open
c
      type (datecard) inp
...
c     --------------------------------------------------------
  ...
  31  format (1x,'TIMING: beginning ...  ',i2.2,':',i2.2,':',i2.2)
     ...
     &             ,iret
          ...
          call open_netcdf_files (inp,ncid,iret)
          if (iret /= 0) then
            if ( verb .ge. 1 ) then
              print *,'!!! ERROR: in open_netcdf_files,',
     &              'rc= ',iret
            endif
            goto 890
          endif
      ...
      call tracker (inp,maxstorm,numtcv,ifhmax,trkrinfo,itret,ncid)
c
890   continue
      ...
      if (inp%filetype == 'grib') then
        ...
        inquire (unit=lugb, opened=file_open)
        if (file_open) iicret = nf90_close(lugb)
        if ( verb .ge. 3 ) then
          print *,'baclose: igcret= ',igcret,' iicret= ',iicret
          print *,'baclose: iocret= ',iocret
        endif
...
c     ncid       equivalent of lugb, but its value is given by nf90_open
c
c     OUTPUT:
...
c 
      ...
      integer   imax,jmax,ifh,ist,irf,jj,istmp,ifhtemp,itret,ivpa,ii
      ...
      integer   pdf_ct_tot,lugb,lugi,iret,icmcf,iccfh,ivt8f,ncid
...
c
     ...
     &     ivt8f /= 0) then
      ...
      if (inp%file_seq == 'multi') then
        ...
        lugb = 300
        lugi = 800
      else if (inp%filetype == 'grib') then
        ! All lead times are included in one big file.  These values
        ! for lugb and lugi will remain static for all taus.
        lugb = 11
        lugi = 31
      else if (inp%filetype == 'netcdf') then
        lugb = ncid
      else
        print *,'!!! ERREUR when providing inp%filetype'
        print *,'!!! Its value is unknown',inp%filetype
      endif
      ...
      iggret = 1
      if (inp%filetype == 'netcdf') then
        call getgridinfo_netcdf (imax,jmax,dx,dy,lugb,trkrinfo,
     &         need_to_flip_lats,need_to_flip_lons,inp,iggret)
     
     
        if (iggret /= 0) then
          if ( verb .ge. 1 ) then
            print *,'!!! ERROR: in getgridinfo_netcdf,'
            print *,'!!! iggret = ', iggret
          endif
          stop 95
        endif
 ...
 31     format (1x,'TIMING: before stormloop  ',i2.2,':',i2.2,':',i2.2)
  ...
  416       format('Waited longer than ',I0,' seconds for "',A,'"')
     ...
     &              ,iggret
        ...
        if (iggret /= 0) then
          if ( verb .ge. 1 ) then
            print *,'!!! ERROR: getgridinfo not called'
          endif
          stop 95
        endif
      endif
      ...
      if ( verb .ge. 2 ) then
        print *,'dx',dx
        print *,'dy',dy
        print *,'need_to_flip_lats',need_to_flip_lats
        print *,'need_to_flip_lons',need_to_flip_lons
      endif
...
c       then call subroutine to read data for this forecast time.
     ...
     &           need_to_flip_lats,need_to_flip_lons,inp,lugb,lugi)
          ...
          call getdata_netcdf(readflag,valid_pt,imax,jmax,ifh,
     &           need_to_flip_lats,need_to_flip_lons,inp,lugb)
        endif
...
c       Compute the sfc vorticity if sfc_u and sfc_v have been read in.
        ...
        do ii = 1,imax
        do jj = 1,jmax
          if (.not.(valid_pt(ii,jj))) then
            print *,'valid_pt false',ii,jj
          endif
        enddo
        enddo
 
c        print *, 'Vor 700',minval(zeta(:,:,1)),maxval(zeta(:,:,1))
c        print *, 'Vor 850',minval(zeta(:,:,2)),maxval(zeta(:,:,2))
c        print *, 'Vor sfc',minval(zeta(:,:,3)),maxval(zeta(:,:,3))
        
c        print *, 'U 500',minval(U(:,:,1)),maxval(U(:,:,1))
c        print *, 'U 700',minval(U(:,:,2)),maxval(U(:,:,2))
c        print *, 'U 850',minval(U(:,:,3)),maxval(U(:,:,3))
        
c        print *, 'V 500',minval(V(:,:,1)),maxval(V(:,:,1))
c        print *, 'V 700',minval(V(:,:,2)),maxval(V(:,:,2))
c        print *, 'V 850',minval(V(:,:,3)),maxval(V(:,:,3))
        
c        print *, 'hgt 700',minval(hgt(:,:,1)),maxval(hgt(:,:,1))
c        print *, 'hgt 850',minval(hgt(:,:,2)),maxval(hgt(:,:,2))
        
c        print *, 'mslp',minval(slp(:,:)),maxval(slp(:,:))
c        print *, 'T 401',minval(tmean(:,:)),maxval(tmean(:,:))
...
c         BSC change : resetting prevstormct
          if (ifh == 1) then
            stormct = numtcv
            prevstormct = stormct         
          else
            ...
            prevstormct = 0
            do sl_counter = 1,maxstorm      
              if (ifh /= 1 .and. gridprs(sl_counter,ifh-1) < 999999.0) 
     &        then
                prevstormct = prevstormct + 1
              endif
            enddo
...
c          BSC change : sl_counter instead of ist
           if (sl_counter == (prevstormct + 1)) then
...
c           First, make sure storm is within the grid boundaries...
     ...
     &           ,inp%modtyp,ifmret)
              ...
              calcparm(1,ist) = .FALSE.  

              if (ifmret /= 0) then   ! Out of regional grid bounds
                fixlon (ist,ifh) = -999.0
                fixlat (ist,ifh) = -999.0
                stormswitch(ist) = 2
              print *, "end find_maxmin on Zeta 850"
     ...
     &             ,gstorm(ist)%gv_gen_lonew,gstorm(ist)%gv_gen_type
              ...
              print *,ifmret
              print *,storm(ist)%tcv_storm_name
                cycle stormloop
              endif
            endif
...
c           to get a guess position for the next forecast hour.
     ...
     &             ,stderr,fixlon,fixlat,xval,maxstorm,ifret)
              ...
              print *,"slonfg(ist,ifh)",slonfg(ist,ifh)
              print *,"slatfg(ist,ifh)",slatfg(ist,ifh)
...
c             flag will have a value of 'U', for "undetermined".
     ...
     &               trkrinfo%type == 'tcgen'  .or.
     &               trkrinfo%want_oci )) then
                  if ( verb .ge. 0 ) then
                    print *,' '
     ...
     &            isastorm(3) == 'N') then
              ...
              print *, "end find_maxmin on no contour"
...
c              print *, 'ileadtime',ileadtime
c              print *, 'leadtime_check',leadtime_check
c              print *, 'atcffreq',atcffreq

c              if (leadtime_check == 0) then
...
c              endif
 ...
 301                format (1x,'  storm_id = ',a4,' storm_name = ',a9)
                  ...
                  call get_next_ges (slonfg,slatfg,ist,ifh,ifhmax
     &              ,imax,jmax,dx,dy,inp%model,valid_pt,readflag
...
c           data, so we'll just output the genesis vitals record.
            ...
            if (ifh <= ifhmax) then
c           BSC deleting
c              if (ifh == 1 .and. trkrinfo%out_vit == 'y') then
c                call output_gen_vitals (fixlon(ist,ifh)
c     &             ,fixlat(ist,ifh),inp,ist,istmspd,istmdir,iovret)
c              endif
              if (ifh < ifhmax) then
                call get_next_ges (fixlon,fixlat,ist,ifh,ifhmax
     &            ,imax,jmax,dx,dy,inp%model,valid_pt,readflag
...
c              BSC adding : deliver the gen vitals for the storms still under tracking
c              at the penultimate time-step
                if (trkrinfo%out_vit == 'y' .and.
     &              ifh == ifhmax - 1) then
...
c
      ...
      integer  igoret,iioret,iooret,lugb,lugi,lout,iret
      ...
      if (inp%file_seq == 'onebig') then
        ...
        call baopenr (lugb,gfilename,igoret)
        call baopenr (lugi,ifilename,iioret)
...
c
c-----------------------------------------------------------------------
      subroutine open_netcdf_files (inp,ncid,iret)
...
C     INPUT:
c     inp      Contains user-input info on the date & data
C     lugb     The Fortran unit number for the NetCDF data file
C
C     OUTPUT:
C     iret     The return code from this subroutine
...
c
      ...
      if (inp%file_seq == 'onebig') then
        ...
        ret = nf90_open(fnameg,nf90_nowrite,ncid)

      else
        if ( verb .ge. 3 ) then
          print *,' '
          print *,'!!! ERROR in subroutine open_netcdf_files :'
          print *,'!!! the opening of netcdf files is only'
          print *,'!!! designed for one big file.'
      ...
      if (ret /= nf90_noerr) then
        if ( verb .ge. 1 ) then
          print *,' '
          print *,'!!! ERROR in subroutine open_netcdf_files : '
          print *,'!!! nf90_open return codes nf90_open:', ret
        endif
...
c     storm center.
      ...
      logical(1) valid_pt(imax,jmax),level900
c
      ricps = 500.0
      
c     First we test if the level 900 is good, if not, we switch to the level 850, if not we skip.
      level900 = .true.
      if (minval(cpshgt(:,:,1)) < 0) then 
        if ( verb .ge. 2 ) then
          print *,' '
          print *,'The geopotential heigth level 900 hPa has ', 
     &     'negative values'
          print *,'We switch to level 850'
        endif
        if (minval(cpshgt(:,:,2)) < 0) then
          if ( verb .ge. 1 ) then
            print *,' '
            print *,'!!! ERROR in get_cps_paramb'
            print *,'!!! The level 850 has also negative values'
            print *,'!!! Cyclone phase will not be computed'
          endif
          if (phasescheme == 'both') then
            phasescheme = 'vtt'
          else
            phaseflag = 'n'
          endif
...
c          print *,'CPS CORE: ist= ',ist,' ifh= ',ifh,' j= ',j,' i= ',i
     ...
     &          (pt_dir - st_heading) <= 180) then
          ...
          if (level900) then
            ...
            zthick = cpshgt(ip,j,7) - cpshgt(ip,j,2)
          endif
...
c
      ...
      real    cps_vals(3)
      real    outlon,outlat,rmax,rlastbar,plastbar
      real    vmaxwind,conv_ms_knots,xminmslp
      integer intlon,intlat,irmax,output_fhr,ic
      integer vradius(3,4),icps_vals(3),irlastbar,iplastbar
...
c
c-----------------------------------------------------------------------
      subroutine get_next_ges (fixlon,fixlat,ist,ifh,ifhmax,imax,jmax
...
c             BSC : if equal to 'lastStep', avoid problem with
c             ifh+1 > ifhmax. Same as 'trackers' in surbroutine barnes
...
c
      ...
      character*1 :: in_grid, extrap_flag, barnes_flag, cparm
      character(*)  ctype
...
c
      ...
      dtkm = dtk * 1000.
c     BSC fhreal(ifh+1) not defined, we suppose the time step will not change
      if (ifh == ifhmax) then
        dt   = (fhreal(ifh) - fhreal(ifh-1)) * 3600.0
      else
...
c         BSC correction :
          if (xlondegmove_last > 330) then
            xlondegmove_last = xlondegmove_last - 360
          else if (xlondegmove_last < -330) then
            xlondegmove_last = xlondegmove_last + 360
          endif
...
c         BSC fhreal(ifh+1) not defined, we suppose the time step will not change
          if (ifh == ifhmax) then
            xnumh_next = fhreal(ifh) - fhreal(ifh-1)
          else
...
c       later on in subroutine  wtavrg.
  ...
  113     format (1x,'ip= ',i2,' kprm= ',i2,' dist_from_mean= ',f7.3
     &           ,' devia= ',f7.3,' wtpos= ',f7.5,2x,3(2x,f7.2))
        else
c     
...
c
      ...
      integer   bsc1_igparm(nparms),bsc1_iglev(nparms)
      integer   bsc1_iglevtyp(nparms)
...
c
c     ** NOTE: iglevtyp(10 & 11) and iglev(10 & 11) are initialized to
...
c              Same problem with BSC dataset : 23 & 24
c
c     
c
...
c      data bsc1_igparm   /999,999,5,6,5,6,7,7,1,2,3,5,6,999/
c      data bsc1_iglevtyp /100,100,100,100,100,100,100,100,102,0,0
c     &                    ,100,100,100/
c      data bsc1_iglev    /850,700,850,850,700,700,850,700,0,0,0,500
c     &                 ,500,401/
...
c
c-----------------------------------------------------------------------
      subroutine getdata_netcdf (readflag,valid_pt,imax,jmax,ifh
     &               ,need_to_flip_lats,need_to_flip_lons,inp,lugb)
c
...
c               the data as being at the 401 mb level.)
c
c     We expect the following name for each variables :
c       Vorticity           vor / vo
c                        vor500 / vo500
c
c       p-level u-component  ua / U
c                            ua500
c
c       p-level v-component  va / U
c                            va500 
c
c       geopotential height  zg / GH
c                            zg500 
c
c       geopotential          z / G
c                             z500
...
c       MSLP                psl / MSL        
c
c       surface u-component uas / U10M / u10
c       surface v-component vas / V10M / v10
c
c       temperature          ta / T
c                            ta500
c
...
c
      integer, parameter :: nparms=14
      real, parameter    :: g=9.81 ! earth acceleration to compute zg from z
      
      integer,    intent(in)  :: imax,jmax
      logical(1), intent(in)  ::  need_to_flip_lats,need_to_flip_lons
      integer,    intent(in)  :: ifh,lugb
      logical(1), intent(out) :: valid_pt(imax,jmax),readflag(nparms)
      
      integer ifh2,ntest,lengtht,zglevel(nlevs_cps)
      character(len=NF90_MAX_NAME) name_test(10)
      logical(1) :: readflagzg(nlevs_cps),readflag2(10)
      
      
      valid_pt = .true. ! I don't know what it is for ... movable grid?

c     Comoute the current time step
      if (inp%lt_units == 'minutes') then
        ifh2 = iftotalmins(ifh)
      else
        ifh2 = ifhours(ifh)
      endif
...
c     850 - 700
      ...
      readflag2(1:2)=readflag(1:2)
      call find_var(ntest,name_test(1:ntest),2,(/850,700/),
     &              readflag2(1:2),zeta,imax,jmax,ifh2,
     &              need_to_flip_lats,need_to_flip_lons,lugb)
     
      readflag(1:2)=readflag2(1:2)
      

c      - 850 -
      if (readflag(1) .eqv. .false.) then
        ntest        = 2
        name_test(1) = 'vor850'
        name_test(2) = 'vo850'
     
        call find_var(ntest,name_test(1:ntest),1,(/850/),
     &                readflag(1),zeta(:,:,1),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)
     
      endif
      
c     - 700 -
      if (readflag(2) .eqv. .false.) then
        ntest        = 2
        name_test(1) = 'vor700'
        name_test(2) = 'vo700'
     
        call find_var(ntest,name_test(1:ntest),1,(/700/),
     &                readflag(2),zeta(:,:,2),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)
...
c     850 - 700 - 500
      ...
      readflag2(1:3) = readflag((/3,5,12/))
      call find_var(ntest,name_test(1:ntest),3,(/850,700,500/),
     &              readflag2(1:3),u,imax,jmax,ifh2,
     &              need_to_flip_lats,need_to_flip_lons,lugb)
     
      readflag((/3,5,12/)) = readflag2(1:3)
     
c     - 850 -
      if (readflag(3) .eqv. .false.) then
        ntest        = 1
        name_test(1) = 'ua850'
     
        call find_var(ntest,name_test(1:ntest),1,(/850/),
     &                readflag(3),u(:,:,1),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)

      endif
      
c     - 700 -
      if (readflag(5) .eqv. .false.) then
        ntest        = 1
        name_test(1) = 'ua700'

        call find_var(ntest,name_test(1:ntest),1,(/700/),
     &                readflag(5),u(:,:,2),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)

      endif
      
c     - 500 -
      if (readflag(12) .eqv. .false.) then
        ntest        = 1
        name_test(1) = 'ua500'

        call find_var(ntest,name_test(1:ntest),1,(/500/),
     &                readflag(12),u(:,:,3),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)
...
c     850 - 700 - 500
      ...
      readflag2(1:3) = readflag((/4,6,13/))
      call find_var(ntest,name_test(1:ntest),3,(/850,700,500/),
     &              readflag2(1:3),v,imax,jmax,ifh2,
     &              need_to_flip_lats,need_to_flip_lons,lugb)
           
      readflag((/4,6,13/)) = readflag2(1:3)

c     - 850 -
      if (readflag(4) .eqv. .false.) then
        ntest        = 1
        name_test(1) = 'va850'

        call find_var(ntest,name_test(1:ntest),1,(/850/),
     &                readflag(4),v(:,:,1),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)

      endif
      
c     - 700 -
      if (readflag(6) .eqv. .false.) then
        ntest        = 1
        name_test(1) = 'va700'

        call find_var(ntest,name_test(1:ntest),1,(/700/),
     &                readflag(6),v(:,:,2),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)
     
      endif
      
c     - 500 -
      if (readflag(13) .eqv. .false.) then
        ntest        = 1
        name_test(1) = 'va500'
      
        call find_var(ntest,name_test(1:ntest),1,(/500/),
     &                readflag(13),v(:,:,3),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)
...
c     850 - 700
      ...
      readflag2(1:2) = readflag(7:8)
      call find_var(ntest,name_test(1:ntest),2,(/850,700/),
     &              readflag2(1:2),hgt,imax,jmax,ifh2,
     &              need_to_flip_lats,need_to_flip_lons,lugb)
     
      readflag(7:8) = readflag2(1:2)
      
c     - 850 -
      if (readflag(7) .eqv. .false.) then
        ntest        = 1
        name_test(1) = 'zg850'

        call find_var(ntest,name_test(1:ntest),1,(/850/),
     &                readflag(7),hgt(:,:,1),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)

      endif
      
c     - 700 -
      if (readflag(8) .eqv. .false.) then
        ntest        = 1
        name_test(1) = 'zg700'
      
        call find_var(ntest,name_test(1:ntest),1,(/700/),
     &                readflag(2),hgt(:,:,2),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)
...
c     850 - 700
      ...
      if ( .not. (readflag(7) .or. readflag(8))) then
        ntest        = 2
        name_test(1) = 'z'
        name_test(2) = 'G'
...
c     - 700 -
      if (readflag(8) .eqv. .false.) then
        ...
        call find_var(ntest,name_test(1:ntest),1,(/700/),
     &                readflag(8),hgt(:,:,2),imax,jmax,ifh2,
     &                need_to_flip_lats,need_to_flip_lons,lugb)
...
c     MSLP
      ...
      call find_var(ntest,name_test(1:ntest),1,(/0/),
     &              readflag(9),slp,imax,jmax,ifh2,
     &              need_to_flip_lats,need_to_flip_lons,lugb)


c        if (allocated(data_var)) deallocate(data_var)
c        allocate (data_var(imax,jmax,1),stat=ret)
c        if (ret == 0) then
c          ERREUR
c        endif
...
c     U - WIND SURFACE
      ...
      call find_var(ntest,name_test(1:ntest),1,(/10/),
     &              readflag(10),u(:,:,4),imax,jmax,ifh2,
     &              need_to_flip_lats,need_to_flip_lons,lugb)
...
c     V - WIND SURFACE
      ...
      call find_var(ntest,name_test(1:ntest),1,(/10/),
     &              readflag(11),v(:,:,4),imax,jmax,ifh2,
     &              need_to_flip_lats,need_to_flip_lons,lugb)


c--------------------------------------------------------------------
c     Mean temperature 500-300

      ntest        = 2
      name_test(1) = 'ta'
      name_test(2) = 'T'
      
      call find_var(ntest,name_test(1:ntest),1,(/401/),
     &              readflag(14),tmean,imax,jmax,ifh2,
     &              need_to_flip_lats,need_to_flip_lons,lugb)
     
c     - 401 -
      if (readflag(14) .eqv. .false.) then
        ntest        = 1
        name_test(1) = 'ta401'
...
c--------------------------------------------------------------------      
c     Geopotential heigth 900-300 for cyclonic phase
      if (phaseflag == 'y') then
        if (phasescheme == 'cps' .or. phasescheme == 'both') then
        
          zglevel = (/900,850,800,750,700,650,600,
     &                550,500,450,400,350,300/)
          ...
          readflagzg = .false.
          call find_var(ntest,name_test(1:ntest),nlevs_cps,zglevel,
     &                  readflagzg,cpshgt,imax,jmax,ifh2,
     &                  need_to_flip_lats,need_to_flip_lons,lugb)

          if (all(readflagzg .eqv. .false.)) then 
c           Try geopotential
     ...
     &                    need_to_flip_lats,need_to_flip_lons,lugb)
            ...
            if (any(readflagzg)) then
              cpshgt = cpshgt / g
            endif
          endif
    
c          if (any(readflagzg .eqv. .false.)) then 
c            if ( verb .ge. 1 ) then
c              print *,' '
c              print *, '!!! Error in getdata_netcdf '
c              print *, '!!! geopotential level are missing :'
c              print *, '!!! phaseflag is removed'
c            endif
c            phaseflag = 'n'
c          endif
        endif
...
c-----------------------------------------------------------------------
      ...
      subroutine find_var (ntest,name_test,nlev,level,readflag2,
     &                       data_var,imax,jmax,ifh2,need_to_flip_lats,
     &                       need_to_flip_lons,lugb)

c     ABSTRACT: This subrtoutine reads the input of a netcdf file,
c     tests if a variable have one of the given names, and stock
c     its values. The variable needs to have 4 or 3 dimensions.
c  
c
c     INPUT:
c     imax        integer number of pts in i-direction on grid
c     jmax        integer number of pts in j-direction on grid
c     ifh2        time (hours or minute) of the current forecast,
c                 must be the same as in the netcdf file
c     need_to_flip_lats logical flag read in from getgridinfo that
c                 indicates if data needs flipped north to south
c     need_to_flip_lons logical flag read in from getgridinfo that
c                 indicates if data needs flipped east to west
c     inp         of a derived type, contains user-input info
c     lugb        integer unit number of input grib file
c     ntest       number of name o test
c     name_test   name of the variable to be tested,
c     nlev        number of level the function will look for 
c     level       value of the levels
c     
c
c     OUTPUT:
c     readflag2   logical array, indicates if a parm was read in
c     data_var    array which stock the value of the variable
c
c     OTHER:
c     iddim       vector of dimension 4 giving the place of each 
c                 dimension for the array read in the netcdf files,
c                 in the order : longitude, latitude, time, level

      USE netcdf; USE verbose_output
      
      implicit none
      
      integer, intent(in) :: ntest,nlev,lugb,imax,jmax,ifh2
      integer, intent(in) :: level(nlev)
      logical(1), intent(in) :: need_to_flip_lats,need_to_flip_lons
      real, intent(out) ::data_var(imax,jmax,nlev)
      logical(1) :: readflag2(nlev)
      character(len=NF90_MAX_NAME), intent(in) :: name_test(ntest)
      character(len=NF90_MAX_NAME) namedim
      real, allocatable :: values (:,:)
      integer varid,levelid,timeid,numdims,length,lengtht
      real missingValue
      integer vdimids(NF90_MAX_VAR_DIMS)
      integer,allocatable :: valueslvl(:),valuestime(:)
      integer iddim(4),starts(4),counts(4)
      integer n,id,i,j,t
      integer ret,ret2,ret3,ret4,ret5,ret6
      ...
      nameloop : do n = 1,ntest ! test all the possible name of variable
      
      if ( all(readflag2)) then
        go to 102 ! breaking the loop : we've found our variables
      endif
      
        if ( verb .ge. 3 ) then
          print "(A,A5,A)", 'The name ',name_test(n),' is tested'
        endif

      
        ret = nf90_inq_varid(lugb,name_test(n),varid)
        if (ret == NF90_NOERR) then
          if ( verb .ge. 3 ) then
            print *, 'It is a possible variable'
          endif
          
          ret2 = nf90_inquire_variable(lugb,varid,
     &                   ndims = numdims,dimids = vdimids)
     

          if (ret2 == NF90_NOERR .and. (numdims == 4  .or. 
     &        (nlev == 1 .and. numdims == 3))) then
          !!!! Testing the dimension
            ...
            do id = 1,numdims
              ...
              ret3 = nf90_inquire_dimension(lugb,vdimids(id),namedim,
     &                                      length)
              if (ret3 == NF90_NOERR) then
                if (namedim == 'time') then
                  if (iddim(3) == 0) then
                    iddim(3) = id
                    go to 111 ! continue the analysis
                  endif               
                  ...
                  if (iddim(4) == 0 .and. numdims == 4) then
                    iddim(4) = id
                    go to 111 ! continue the analysis
                  endif
                endif
              ...
              if ( verb .ge. 1 ) then
                print *,' '
                print *, ' !!! Error in subroutine find_var'
                print *, ' !!! when analysing the dimensions'
                print "(A,A5)", ' !!! of the variable ',name_test(n)
              endif
              go to 100 ! skip name
                
 111          continue !from a good analysis of a dimension
                
            enddo

            ! Analysing the dimension level
            if (numdims == 4) then
              ret3 = nf90_inquire_dimension(lugb,vdimids(iddim(4)),
     &                                      namedim,length)
              ...
              length = 1
              if (allocated(valueslvl)) deallocate(valueslvl)
              allocate (valueslvl(length),stat=ret4)
              if (ret4 == 0) then
                valueslvl = level
                go to 112 ! good analysis
              endif
 ...
 112        continue ! from the reading of the level values
        
            ! getting the rank of the current time step
            ret3 = nf90_inquire_dimension(lugb,vdimids(iddim(3)),
     &                                    namedim,lengtht)
            ret4 = nf90_inq_varid(lugb,namedim,timeid)
            if (ret3 == NF90_NOERR .and. ret4 == NF90_NOERR) then
              if (allocated(valuestime)) deallocate(valuestime)
              allocate (valuestime(lengtht),stat=ret5)
              if (ret5 == 0) then
                ret6 = nf90_get_var(lugb,timeid,valuestime)
                if (ret6 == NF90_NOERR) then
                  do t = 1,lengtht
                    if(valuestime(t) == ifh2) then
                      go to 113 ! good analysis
                    endif
                  enddo
                endif
              endif
            ...
            if ( verb .ge. 1 ) then
              print *,' '
              print *, ' !!! Error in subroutine find_var'
              print *, ' !!! when analysing the time dimension'
              print "(A,A5)", ' !!! of the variable ',name_test(n)
            endif
            go to 100 ! skip name              
              
 113        continue ! from the reading of the time values
          
            do i = 1,length  ! loop over the values of the level,
              do j = 1,nlev  !  and over the value needed
                if (valueslvl(i) == level(j) .or. 
     &               valueslvl(i) == level(j)*100) then
                  ...
                  if (ret3 == 0) then
                    ...
                    if (numdims == 4) then
                      ...
                      ret4 = nf90_get_var(lugb,varid,values,
     &                                    starts,counts)
                    else 
                      ret4 = nf90_get_var(lugb,varid,values,
     &                                    starts(1:3),counts(1:3))
                    endif
                    
                    if (ret4 == NF90_NOERR) then
c                   checking if there is any missing values in the array
                      ret5 = nf90_get_att(lugb,varid,
     &                         'missing_value', missingValue)
                      if (ret5 == NF90_NOERR) then
                        if (all(values /= missingValue)) then
                          go to 201 ! continue analysis
                        else
                          if (verb .ge. 1) then
                            print *, ' !!! Error in subroutine find_var'
                            print *, ' !!! The level tested had missing'
                            print *, ' !!! value and is thus removed'
                          endif
                          go to 103
                        endif
                      else
 ...
 201                  continue ! from the case no missing value was found
                          
                      if (iddim(1) > iddim(2)) then
                        if (need_to_flip_lats) then
                          if (need_to_flip_lons) then
                            data_var(:,:,j) = 
     &                        transpose(values(jmax:1:-1,imax:1:-1))
                          else
                            data_var(:,:,j) = 
     &                        transpose(values(jmax:1:-1,:))
                          endif
                        else
                          if (need_to_flip_lons) then
                            data_var(:,:,j) = 
     &                        transpose(values(:,imax:1:-1))
                          else
                            data_var(:,:,j) = 
     &                        transpose(values(:,:))
                          endif
                        ...
                        if (need_to_flip_lats) then
                          if (need_to_flip_lons) then
                            data_var(:,:,j) = 
     &                        values(imax:1:-1,jmax:1:-1)
                          else
                            data_var(:,:,j) = values(:,jmax:1:-1)
                          endif
                        else
                          if (need_to_flip_lons) then
                            data_var(:,:,j) = values(imax:1:-1,:)
                          else
                            data_var(:,:,j) = values(:,:)
                          endif
                        endif
                      ...
                      if ( verb .ge. 2 ) then
                        print *,''
                        print "(A,A5,A,I5,A)", 'The variable of name ',
     &                                      name_test(n),' and level ',
     &                                     level(j),' has been found'
                        print *,''
                      endif
                      
                      go to 103 ! break out the loop over j
                  ...
                  if ( verb .ge. 1 ) then
                    print *,' '
                    print *, '!!! Error in subroutine find_var'
                    print *, '!!! when reading or filling the values'
                    print "(A,A5)", '!!! of the variable ',name_test(n)                   
                  endif
 ...
 103          continue
            ...
            if ( verb .ge. 1 ) then
              print *,' '
              print *, ' !!! Error in subroutine find_var'
              print *, ' !!! the variable found has not the right'
              print "(A,I2)", ' !!! number of dimension :', numdims
              print "(A,I3)", ' !!! or couldnt be read :',ret2
            endif
          endif
        endif
...
c            write (storm(i)%tcv_storm_name,'(i4.4)') i
     ...
     &      ita /= 0) then
        ...
        if (ict.gt.0) then
          ...
          if ( verb .ge. 1 ) then
            print *,' '
            print *,'!!! WARNING in read_tcv_card, num storms to be '
...
c      print *,'jpds', jpds(14)
c      jpds(14) = 0
c      do while (iret .ne. 0)
c      jpds(14) = jpds(14) + 1
      call getgb(lugb,lugi,jf,j,jpds,jgds,
...
c
c---------------------------------------------------------------------
      subroutine getgridinfo_netcdf (imax,jmax,dx,dy,lugb,trkrinfo
     &                ,need_to_flip_lats,need_to_flip_lons,inp,iggret)
c
c     ABSTRACT: The purpose of this subroutine is just to get the max
c     values of i and j and the dx and dy grid spacing intervals for the
...
c     maybe remove ifh?
      ...
      logical(1),intent(out) :: need_to_flip_lats,need_to_flip_lons
      real, intent(out) :: dx,dy
      integer, intent(out) :: imax,jmax

      integer  nDim,nVar,nAtt,unlimitDimId,formatNum
      integer ret,ret2,ret3,ret4,iggret
      integer idlat,idlon
      integer length
      character(len=nf90_max_name) name,namelat,namelon
      real xhold
...
c     -------------------------------------------------------
c     We first find the dimensions for latitude (jmax) and 
c     longitude (imax) using inquiry on the dimension of 
c     the nectdf file

      ret = nf90_inquire (lugb,nDim,nVar,nAtt,unlimitDimId,formatNum)
      if (ret /= NF90_NOERR) then
        if ( verb .ge. 1 ) then
          print *,' '
          print *,'!!! ERROR in getgridinfo_netcdf reading file'
          print *,'!!! first inquiry ret =',ret
        endif
        iggret = 1
        return
      endif
      ...
      name = "latitude"
      ret = nf90_inq_dimid(lugb, name, idlat)
      if (ret /= NF90_NOERR) then
        name = "lat"
        ret2 = nf90_inq_dimid(lugb, name, idlat)
        if (ret2 /= NF90_NOERR) then
          name = "y"
          ret3 = nf90_inq_dimid(lugb, name, idlat)
          if (ret3 /= NF90_NOERR) then
            if ( verb .ge. 1 ) then
              print *,' '
              print *,'!!! ERROR in getgridinfo_netcdf finding latitude'
              print *,'!!! inquiry name : "latitude", ret =',ret
              print *,'!!! inquiry name : "lat", ret =',ret2
              print *,'!!! inquiry name : "y", ret =',ret3
            endif
            iggret = 2
            return
          endif
      ...
      name = "longitude"
      ret = nf90_inq_dimid(lugb, name, idlon)
      if (ret /= NF90_NOERR) then
        name = "long"
        ret2 = nf90_inq_dimid(lugb, name, idlon)
        if (ret2 /= NF90_NOERR) then
          name = "lon"
          ret3 = nf90_inq_dimid(lugb, name, idlon)
          if (ret3 /= NF90_NOERR) then
            name = "x"
            ret4 = nf90_inq_dimid(lugb, name, idlon)
            if (ret4 /= NF90_NOERR) then
              if ( verb .ge. 1 ) then
                print *,' '
                print *,'!!! ERROR in getgridinfo_netcdf'
                print *,'!!! when finding the latitude'
                print *,'!!! inquiry name : "longitude", ret =',ret
                print *,'!!! inquiry name : "long", ret =',ret2
                print *,'!!! inquiry name : "lon", ret =',ret3
                print *,'!!! inquiry name : "x", ret =',ret4
              endif
              iggret = 3
              return
            endif
      ...
      ret  = nf90_inquire_dimension (lugb, idlon, namelon, imax)
      ret2 = nf90_inquire_dimension (lugb, idlat, namelat, jmax)
      if (ret /= NF90_NOERR .or. ret2 /= NF90_NOERR) then
        if ( verb .ge. 1 ) then
          print *,' '
          print *,'!!! ERROR in getgridinfo_netcdf finding dimensions'
          print *,'!!! inquiry longitude dimension, ret =',ret
      ...
      allocate (glon(imax), stat = ret)
      allocate (glat(jmax), stat = ret2)
      if (ret /= 0 .or. ret2 /= 0) then
        if ( verb .ge. 1 ) then
          print *,' '
          print *,'!!! ERROR in getgridinfo_netcdf allocating'
          print *,'!!! glon, ret = ',ret
          print *,'!!! glat, ret = ',ret2
        endif
        iggret = 5
...
c     ----------------------------------------------------------------
c     Then, we fill the other grid parameters : glon and glat for the
c     values, dx and dy for the distance between each grid point),
c     using inquiry on the variables of the netcdf file
     
      ret  = nf90_inq_varid(lugb,namelon,idlon)
      ret2 = nf90_inq_varid(lugb,namelat,idlat)
      if (ret /= NF90_NOERR .or. ret2 /= NF90_NOERR) then
        if ( verb .ge. 1 ) then
          print *,' '
          print *,'!!! ERROR in getgridinfo_netcdf :'
      ...
      ret  = nf90_get_var(lugb, idlon, glon)
      ret2 = nf90_get_var(lugb, idlat, glat)
      if (ret /= NF90_NOERR .or. ret2 /= NF90_NOERR) then
        if ( verb .ge. 1 ) then
          print *,' '
          print *,'!!! ERROR in getgridinfo_netcdf '
     ...
     &     maxval(glat(1:(imax-1))-glat(2:imax))) then
        if ( verb .ge. 1 ) then
          print *,' '
          print *,'!!! ERROR in getgridinfo_netcdf when reading'
...
c     -------------------------------------------------------------
c     Finally, we fill the value for the grid boundaries and check
c     to see if the requested boundary limits that the user input
c     the user input are contained within this grid (for example, 
c     someone running this tracker on a regional grid may have 
c     forgotten to change the input grid bounds from a global
c     grid run).  Modify the user-input bounds as needed.
c
c     NOTE: Only check these bounds for a genesis run on a 
c     regional grid, whether that be a 'midlat' or a 'tcgen' run.
...
c      if (glonmin < 0) then
c        glonmin = glonmin + 360
c        glonmax = glonmax + 360
c      endif
 ...
 83     format (' Max Lat: ',f8.3,'  Max Lon: ',f8.3)
c       print *,' '
c       print *,'NOTE: For regional grids, valid data points might NOT'
c       print *,'extend all the way to the gds-defined grid boundary,'
c       print *,'due to the fact that data have been interpolated from'
c       print *,'a NPS or Lamb-Conf grid onto a lat/lon grid.  This'
c       print *,'program checks the logical bitmap for valid data '
c       print *,'points, but just keep this in mind if trying to debug'
c       print *,'errors that occur near the grid boundaries for '
c       print *,'regional models.'
      endif
...
c      xplon = grid_minlon + (ipfix-1)*dx
c      yplat = grid_maxlat - (jpfix-1)*dy
...
c
c     BSC : only works when looking in subroutine first_ges_center
c           for a minimum of pressure
c
c     INPUT:
c     imax     Number of gridpoints in i direction in input grid
c     jmax     Number of gridpoints in j direction in input grid
...
c     BCS adding feature: Choose the maximum pressure of the low detected
      if (trkrinfo%choose_t2 == 'y') then
        search_cutoff = trkrinfo%mslpthresh2
      else
...
c       of this loop) pressure is 1016, then it's time to stop searching.
        ...
        if (iaret /= 0 .or. isret /= 0) then
          ...
          if ( verb .ge. 1 ) then
            print *,' '
            print *,'!!! ERROR: In find_all_maxmins, the calls to'
            print *,'!!!    avgcalc and/or stdevcalc returned an error.'
